...
module.exports = {
	...
	description: 'Bedava Hesap Al!!!', // Command description
        ...
        if (message.channel.id === config.genChannel) {
            ...
            if (generated.has(message.author.id)) {
                return message.channel.send(
                    ...
                    .setColor(config.color.red)
                    .setTitle('Sakin Ol!')
                    .setDescription('Bu Komutu Tekrar Çalıştırmadan Önce Lütfen Biraz Bekle!')
                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                    .setTimestamp()
                );
            } else {
                    ...
                    if (!error) {
                        ...
                        if (position === -1) {
                            return message.channel.send(
                                ...
                                .setColor(config.color.red)
                                .setTitle('Stok Yok!')
                                .setDescription(`\`${args[0]}\` Servisinde Stok Bulunamadı!`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        .then(message.author.send('Kolayca Kopyala-Yapıştır Yapman İçin:'))
                        ...
                        if (position !== -1) {
                            ...
                            fs.writeFile(filePath, data, function (error) {
                                message.channel.send(
                                    ...
                                    .setColor(config.color.green)
                                    .setTitle('Hesabınız Oluşturuldu!')
                                    .setDescription(`Dm Kutunu Kontrol Et ${message.author}! Mesaj Gelmediyse, Gizlilik Ayarlarını Düzenlemeyi Unutma!`)
                                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                    .setTimestamp()
                                );
                            ...
                            return message.channel.send(
                                ...
                                .setDescription(`\`${args[0]}\` Servis Boş!`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        return message.channel.send(
                            ...
                            .setColor(config.color.red)
                            .setTitle('Generator Hatası!')
                            .setDescription(`Servis \`${args[0]}\` Bulunamadı!`)
                            .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                            .setTimestamp()
                        );
                    };
            ...
            message.channel.send(
                new MessageEmbed()
                .setColor(config.color.red)
                .setTitle('Yanlıs Komut Kullanıyorsun!')
                .setDescription(`Bu Kanalda \`gen\` Komutunu Kullanamazsın! <#${config.genChannel}> Burda Tekrar Dene!`)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );
        };
