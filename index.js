...
client.once('ready', () => {
  ...
  client.user.setActivity(`c!yardım • Valorant Gen`, { type: "WATCHING" }); // Set the bot's activity status
  ...
  if (config.command.notfound_message === true && !client.commands.has(command)) {
    return message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(`Kusura Bakma Kral \`${command}\` Komutunu Bulamadım!`)
    ...
    if (config.command.error_message === true) {
      message.channel.send(
        new Discord.MessageEmbed()
          ...
          .setDescription(` \`${command}\` Komutu Yürütülürken Bir Hata Oluştu!`)
          .addField('Error', `\`\`\`js\n${error}\n\`\`\``)
          .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
          .setTimestamp()
      );
